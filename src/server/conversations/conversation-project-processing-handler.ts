...
import { ConversationProjectProcessor } from '../../services/conversations/conversation-project-processor';
import { ConversationProcessingInProgressError } from '../../services/conversations/conversation-project-processing-repository';
import { SupabaseConversationProjectProcessingRepository } from '../../services/conversations/supabase-conversation-project-processing-repository';
...
import { OpenAIConversationProjectAnalyzer } from './openai-conversation-project-analyzer';

declare const process: { env: Record<string, string | undefined> };

type Processor = Pick<ConversationProjectProcessor, 'process'>;
type HandlerOptions = {
...
export async function handleConversationProjectProcessing(
  request: Request,
  ...
  if (!allowedOrigin) return json({ error: 'Processing is unavailable.' }, 500);
  const browser = requestOrigin === allowedOrigin;
  ...
  if (request.method !== 'POST') {
    return json({ error: 'Request rejected.' }, 405, responseOrigin);
  }
  ...
  try {
    ...
    if (error instanceof SupabaseAuthUnavailableError) {
      return json({ error: 'Processing is unavailable.' }, 500, responseOrigin);
    }
    if (!(error instanceof InvalidAccessTokenError)) {
      console.error('Conversation processing authentication failed.');
    }
  ...
  try {
    ...
    conversationId = typeof body.conversationId === 'string' &&
      body.conversationId.trim() && body.conversationId.length <= 300
      ? body.conversationId
      : null;
  } catch {
    conversationId = null;
  ...
  if (!conversationId) {
    return json({ error: 'Conversation ID is required.' }, 400, responseOrigin);
  }
  ...
  try {
    ...
    if (error instanceof ConversationProcessingInProgressError ||
      (error instanceof Error && error.name === 'ConversationProcessingInProgressError')) {
      return json({ status: 'processing' }, 202, responseOrigin);
    }
    console.error('Conversation Project processing failed.', error);
    return json({ error: 'The conversation was saved, but Project organization failed.' }, 502, responseOrigin);
  }
}
...
function createProcessor(
  ...
  return new ConversationProjectProcessor(
    new OpenAIConversationProjectAnalyzer({ apiKey }),
    new SupabaseConversationProjectProcessingRepository(() => client),
    new SupabaseProjectRepository(() => client, context.userId),
  );
}
