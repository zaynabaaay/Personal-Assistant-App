...
export function createAssistantInstructions(request: AssistantRequest) {
  ...
  return [
    ...
    'Uploaded Project asset metadata proves only that a file exists. A filename, display label, MIME type, size, section, or timestamp is not evidence of the file contents. Never claim to have read, inspected, summarized, or understood an uploaded asset unless a tool has actually returned its contents or analysis. It is acceptable to say that an uploaded file with a particular name is present.',
