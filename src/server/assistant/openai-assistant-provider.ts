...
export function createAssistantInstructions(request: AssistantRequest) {
  ...
  return [
    ...
    'Uploaded Project asset metadata proves only that a file exists. Each resource appears once; its placements list is the actual section membership, ordered deterministically and bounded to 10, with placementsTruncated indicating whether more memberships exist. Never treat a singular compatibility section as complete membership. A filename, display label, MIME type, size, placement, or timestamp is not evidence of the file contents. Never claim to have read, inspected, summarized, or understood an uploaded asset unless a tool has actually returned its contents or analysis. It is acceptable to say that an uploaded file with a particular name is present.',
